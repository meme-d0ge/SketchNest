...
import {rect, rotate, bounds, polyline, ellipse, translate} from "@thi.ng/geom";
...
export function getBounds(item: BoardElementOptionalId) {
    switch (item.type) {
        ...
        case 'line': {
            if (item.data.points.length < 2 || item.data.points.length % 2 !== 0) return null;
            const newArrPoints = []
            for (let i = 0; i < item.data.points.length; i = i + 2) {
                newArrPoints.push([item.data.x + item.data.points[i], item.data.y + item.data.points[i+1]])
            }
            const angle = (item.data.rotation * Math.PI) / 180;
            ...
            const pivot = [item.data.x + (localBounds.minX + localBounds.maxX) / 2, item.data.y + (localBounds.minY + localBounds.maxY) / 2];
